export const socials = [
  { name: "my projects", href: "https://github.com/EzequielMadrid" },
  { name: "chat me", href: "https://www.linkedin.com/in/ezequielmadrid97/" },
];

export const serviceRows = {
  row1: ["DApps", "Sockets", "SaaS", "Docker"],
  row2: ["APIs", "Metaverse", "Web3", "Microservices"],
  row3: ["SPA", "SQL", "Solidity"],
};

export const faqs = [
  {
    question: "What services do you offer?",
    answer:
      "I design and develop modern, scalable web applications, professional systems, and high-performance digital platforms, including SaaS solutions and microservices architectures, all built with a strong focus on user experience, performance, and clean architecture, applied in real-world projects.",
  },
  {
    question: "How can I contact you?",
    answer:
      "You can find me on my LinkedIn via the navigation menu at the top right. I'm committed to clear communication and typically respond within 24 hours.",
  },
  {
    question: "Do you work internationally?",
    answer:
      "Yes, I do. From around the world, providing flexible communication and collaboration across different time zones.",
  },
];

export const myProjects = [
  {
    id: 1,
    title: "InterMinisterial Forum",
    href: "https://inter-ministerial.vercel.app/",
    texture: "/textures/projects/project1.mp4",
  },
  {
    id: 2,
    title: "Corporation Chat",
    href: "https://rooms-bkg7.onrender.com/signin",
    texture: "/textures/projects/project2.mp4",
  },
];
